	 ...
	 ((and (matlab-ltype-code)
	  ...
	  (let ((m (make-marker)))
		...
		(if matlab-fill-strings-flag
			...
			(while (and (re-search-forward "'" m t)
				    (looking-at "''")))
			(forward-char -1)
			(if (not (looking-at "'\\s-*]"))
			    (progn
...
(defun matlab-shell-add-to-input-history (string) 
  "Add string to the input-ring and run comint-input-filter-functions on
it, as is done in comint-send-input."
